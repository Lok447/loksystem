...
| R-08 | P1 | 崩溃观测 | GPU crash 自愈和 Sentry 过滤 | todo | patch-2 |
| R-09 | P1 | 日志诊断 | 反馈/崩溃报告附带更完整日志与环境标签 | todo | patch-2 |
| C-01 | P1 | Agent | 统一 agent 元数据和能力发现 | todo | minor-1 |
| C-02 | P1 | ACP/Aionrs | slash commands 按 agent 动态加载 | todo | minor-1 |
| C-03 | P1 | 权限确认 | 统一工具权限确认、状态更新、结果展示 | todo | minor-1 |
| C-04 | P1 | Team | 团队会话 agent/model/workspace 选择一致性 | todo | minor-1 |
| C-05 | P1 | Cron | 定时任务使用真实 agent/provider/model 信息 | todo | minor-1 |
