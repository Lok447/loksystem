...
<p align="center">
  <a href="../../readme.md">English</a> | <strong>简体中文</strong> | <a href="./readme_tw.md">繁體中文</a> | <a href="./readme_jp.md">日本語</a> | <a href="./readme_ko.md">한국어</a> | <a href="./readme_es.md">Español</a> | <a href="./readme_pt.md">Português</a> | <a href="./readme_tr.md">Türkçe</a> | <a href="./readme_ru.md">Русский</a> | <a href="./readme_uk.md">Українська</a> | <a href="https://www.loksystem.com" target="_blank">官方网站</a>
</p>
...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 为什么选择 LokSystem？](#-为什么选择-loksystem-而非-claude-cowork) ·
[🚀 快速开始](#-快速开始) ·
[💬 社区](#-社区与支持)
...
**LokSystem 不只是个聊天工具。** 它是一个真正的 Cowork 平台，AI Agent 就像您的得力助手，在电脑上帮您处理各种任务——读文件、写代码、查资料、自动化工作流。Agent 的一举一动都在您的掌控之中，透明可见。

|                       | 传统 AI 聊天客户端 | **LokSystem (Cowork)**                                                                                          |
| :-------------------- | :----------------- | :----------------------------------------------------------------------------------------------------------- |
...
LokSystem 自带完整的 AI Agent。不像其他工具需要你手动安装各种 CLI 工具，**LokSystem 装好就能用，开箱即用**。
...
想把文档/表格直接交给 Agent？LokSystem 内置 **[OfficeCLI](https://github.com/iOfficeAI/OfficeCli)**，让 PPT（Morph 转场）、Word（`.docx`）与 Excel（`.xlsx/.xlsm/.csv`）从需求到可交付结果更高效、更稳定。
...
如果你已经在用 Claude Code、Codex、Hermes Agent 或 OpenClaw，LokSystem 会自动发现它们，让你同时和这些 Agent 一起协作——当然，还有内置 Agent。

**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Lok CLI（aionrs，LokSystem 自带的 Rust 后端服务） • Snow CLI • Kiro • Hermes Agent • Cursor Agent 等
...
- **多 Agent 并行执行** — Leader 将任务分解为子任务并委派给并行运行的 Teammate Agent；每个 Teammate 通过 ACP（Agent Communication Protocol，LokSystem 的多 Agent 协调层）、Gemini 或 Aionrs 使用独立模型
- **Leader 统筹编排** — Leader 分配、追踪并汇总结果；支持的后端包括 Claude Code、Codex、Hermes Agent、Gemini、Snow CLI 和 Lok CLI
...
- **支持的后端** — Claude Code、Codex、Gemini、Snow CLI、Lok CLI（aionrs）；其他具备 `mcpCapabilities.stdio` 的 ACP 后端自动支持
...
其他 AI 应用可能只给你个聊天窗口，**但 LokSystem 给你的是完整的 Cowork Agent**。
...
| AWS Bedrock 凭证                         | 通过 Lok CLI（aionrs）的 Bedrock 驱动 Agent |
| Ollama / LM Studio（本地）               | 本地模型 Cowork Agent                        |
| NewAPI 网关                              | 统一访问 20+ 模型                            |

不管用哪个模型，Agent 的能力都一样强大——文件读写、网络搜索、图像生成、工具调用，一个不少。LokSystem 支持 **20+ 个 AI 平台**，云端本地都能用。
...
LokSystem 还支持 [NewAPI](https://github.com/QuantumNous/new-api) 网关服务 — 一个统一的 AI 模型中心，聚合和分发各种大语言模型。在同一界面中灵活切换不同模型，满足各种任务需求。
...
- **三层技能体系** — 内置技能（随 LokSystem 附带）、自定义技能（你自己的）以及扩展技能（第三方扩展贡献）；通过技能指示器按对话启用/禁用
...
LokSystem 内置 **20 个专业助手**，每个都有独特能力，还能通过自定义技能继续扩展：
...
**自定义技能**：在 `skills/` 目录下创建你的专属技能，随时为助手开启或关闭，让 AI 能力无限扩展。技能来源分三层：内置（随 LokSystem 附带）、自定义（你自己的）以及扩展（通过扩展 SDK 贡献）。内置技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。
...
> **设置：** LokSystem 设置 → WebUI 设置 → Channel，配置 Bot Token。
...
> [远程互联网访问教程](https://github.com/iOfficeAI/LokSystem/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **防休眠** — LokSystem 会自动阻止系统休眠，任务激活期间检测唤醒后的漏触发
...
> [图像生成模型配置指南](https://github.com/iOfficeAI/LokSystem/wiki/LokSystem-Image-Generation-Tool-Model-Configuration-Guide)
...
## 🤔 为什么选择 LokSystem 而非 Claude Cowork？
...
LokSystem 是一个**免费开源的 Multi-AI Agent 桌面应用**。相比只能在 macOS 上用、还只能绑定 Claude 的 Claude Cowork，LokSystem 支持全模型、跨平台，是它的全面升级版。

| 维度     | Claude Cowork | LokSystem                                                         |
| :------- | :------------ | :------------------------------------------------------------- |
...
<details>
<summary><strong>问：我需要先安装额外的 CLI 工具吗？</strong></summary>
答：<strong>完全不需要。</strong> LokSystem 自带 AI Agent，装好就能用。用 Google 登录或者输入任意 API 密钥就行。如果你已经装了 Claude Code、Codex 或 Qwen Code 这些受支持的 CLI 工具，LokSystem 会自动发现并集成它们，功能更强大。
</details>

<details>
<summary><strong>问：我可以用 LokSystem 做什么？</strong></summary>
答：LokSystem 就是你的<strong>私有 Cowork 工作空间</strong>。内置 Agent 可以帮你批量整理文件夹、处理 Excel 数据、生成文档、搜索网络、生成图像。通过多 Agent 模式，你还能在同一界面同时使用 Claude Code、Codex 和其他强大的 CLI Agent。
</details>

<details>
<summary><strong>问：它是免费的吗？</strong></summary>
答：LokSystem 完全免费且开源。你可以用 Google 登录免费使用 Gemini，或者用任何你喜欢的 API 密钥。
</details>
...
> **制作了关于 LokSystem 的视频？** [在 X 上告诉我们](https://x.com/LokSystem)，我们会在这里展示！
...
- [📖 完整安装指南](https://github.com/iOfficeAI/LokSystem/wiki/Getting-Started) — 从下载到配置，一步步教你
- [⚙️ LLM 配置指南](https://github.com/iOfficeAI/LokSystem/wiki/LLM-Configuration) — 多平台 AI 模型怎么配置
- [🤖 多 Agent 模式设置](https://github.com/iOfficeAI/LokSystem/wiki/ACP-Setup) — 把终端 AI Agent 集成进来
- [🔌 MCP 工具配置](https://github.com/iOfficeAI/LokSystem/wiki/MCP-Configuration-Guide) — 模型上下文协议服务器设置
- [🌐 WebUI 配置指南](https://github.com/iOfficeAI/LokSystem/wiki/WebUI-Configuration-Guide) — WebUI 完整设置教程

**🎯 使用场景**

- [📁 文件管理](https://github.com/iOfficeAI/LokSystem/wiki/file-management) — 让 AI 帮你整理文件
- [📊 Excel 处理](https://github.com/iOfficeAI/LokSystem/wiki/excel-processing) — AI 驱动的数据处理
- [🎨 图像生成](https://github.com/iOfficeAI/LokSystem/wiki/LokSystem-Image-Generation-Tool-Model-Configuration-Guide) — AI 图像生成
- [📚 更多使用场景](https://github.com/iOfficeAI/LokSystem/wiki/Use-Cases-Overview)

**❓ 支持与帮助**

- [❓ FAQ](https://github.com/iOfficeAI/LokSystem/wiki/FAQ) — 常见问题和解决方案
- [🔧 配置与使用教程](https://github.com/iOfficeAI/LokSystem/wiki/Configuration-Guides) — 完整配置文档
...
- [报告问题](https://github.com/iOfficeAI/LokSystem/issues) — 遇到 bug 或有新功能想法？告诉我们
- [发布更新](https://github.com/iOfficeAI/LokSystem/releases) — 获取最新版本
...
<td>
  <a href="https://packycode.com" target="_blank">PackyCode</a> 是一家可靠高效的 API 中继服务提供商，为 Claude Code、Codex、Gemini 等平台提供中继服务。感谢 PackyCode 为支持 LokSystem 用户低成本使用，为我们的用户提供专属 <a href="https://www.packyapi.com/register?aff=loksystem" target="_blank">9折优惠</a>，付款时使用优惠码 <code>loksystem</code> 可立减 10%。
</td>
</tr>
<tr>
<td width="170" align="center">
  <a href="https://atomgit.com/iOfficeAI/LokSystem" target="_blank">AtomGit</a>
</td>
<td>
  <a href="https://atomgit.com/iOfficeAI/LokSystem" target="_blank">AtomGit</a> — 面向全球开发者的开源社区与代码托管平台。LokSystem 已在 AtomGit 上线，欢迎访问。
</td>
</tr>
</table>
...
[报告 Bug](https://github.com/iOfficeAI/LokSystem/issues) · [请求功能](https://github.com/iOfficeAI/LokSystem/issues)
