...
git clone https://github.com/iOfficeAI/LokSystem.git
cd LokSystem
...
When you have two clones of the repository (e.g. `LokSystem` and `LokSystem-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `LOKSYSTEM_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`LokSystem-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.loksystem-dev-2`, `~/.loksystem-config-dev-2`)
...
LokSystem uses **electron-vite** for fast bundling:
