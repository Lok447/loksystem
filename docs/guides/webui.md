# LokSystem WebUI Mode - Startup Guide

LokSystem supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts LokSystem with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use LokSystem from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\LokSystem\LokSystem.exe" --webui

# Or if LokSystem is in your PATH
LokSystem.exe --webui
```
...
```bash
# Using full path
/Applications/LokSystem.app/Contents/MacOS/LokSystem --webui

# Or using open command
open -a LokSystem --args --webui
```

### Method 2: Create Shell Script

Create `start-loksystem-webui.sh`:
...
```bash
chmod +x start-loksystem-webui.sh
./start-loksystem-webui.sh
```
...
chmod +x LokSystem-*.AppImage

# Run with --webui flag
./LokSystem-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/loksystem-webui.desktop`:

```ini
[Desktop Entry]
Name=LokSystem WebUI
Comment=Start LokSystem in WebUI mode
Exec=/opt/LokSystem/loksystem --webui
Icon=loksystem
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/loksystem-webui.desktop
```
...
Create `~/bin/start-loksystem-webui.sh`:
...
```bash
chmod +x ~/bin/start-loksystem-webui.sh
```
...
Create `/etc/systemd/system/loksystem-webui.service`:

```ini
[Unit]
Description=LokSystem WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/LokSystem/loksystem --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable loksystem-webui.service
sudo systemctl start loksystem-webui.service

# Check status
sudo systemctl status loksystem-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run LokSystem in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making LokSystem accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running LokSystem WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/LokSystem/issues/217)
...
#### 4. Download and Install LokSystem

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/LokSystem/releases
wget https://github.com/iOfficeAI/LokSystem/releases/download/vVERSION/LokSystem_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/LokSystem/releases/download/vVERSION/LokSystem_VERSION_arm64.deb

# Install the package
apt install -y ./LokSystem_*.deb

# Verify installation
which LokSystem
```

#### 5. Launch LokSystem WebUI

```bash
# Start LokSystem in WebUI mode with no-sandbox flag
LokSystem --no-sandbox --webui
```
...
To access LokSystem from other devices on your local network:

```bash
# Start with --remote flag
LokSystem --no-sandbox --webui --remote
...
LokSystem --no-sandbox --webui --port 8080
```
...
chmod +x /opt/LokSystem/loksystem
```

#### Out of Memory

LokSystem requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if LokSystem is running: look for "Server started" message
...
4. **Keep device charged** - Running LokSystem consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **LokSystem Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch LokSystem quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-loksystem.sh << 'EOF'
#!/bin/bash
...
```bash
proot-distro login ubuntu -- bash -c "LokSystem --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/LokSystem/issues/217)
...
```cmd
LokSystem.exe --webui --remote
```

### macOS

```bash
/Applications/LokSystem.app/Contents/MacOS/LokSystem --webui --remote
```

### Linux

```bash
loksystem --webui --remote
```
...
netsh advfirewall firewall add rule name="LokSystem WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add LokSystem
...
which loksystem
# or
find /opt -name "loksystem" 2>/dev/null
```
...
```powershell
& "C:\Program Files\LokSystem\LokSystem.exe" --webui 2>&1 | Tee-Object -FilePath loksystem.log
```

**macOS/Linux:**

```bash
/path/to/loksystem --webui 2>&1 | tee loksystem.log
```
...
export LOKSYSTEM_PORT=8080

# Allow remote access without passing --remote
export LOKSYSTEM_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as LOKSYSTEM_ALLOW_REMOTE=true)
export LOKSYSTEM_HOST=0.0.0.0

# Then start the application
loksystem --webui

# You can also pass the port directly via CLI
loksystem --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/LokSystem/webui.config.json`                     |
| macOS    | `~/Library/Application Support/LokSystem/webui.config.json` |
| Linux    | `~/.config/LokSystem/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\LokSystem\LokSystem.exe" --resetpass

# Or for a specific user
"C:\Program Files\LokSystem\LokSystem.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/LokSystem.app/Contents/MacOS/LokSystem --resetpass

# Or for a specific user
/Applications/LokSystem.app/Contents/MacOS/LokSystem --resetpass username
```
...
# Or for a specific user
loksystem --resetpass username

# Or using full path
/opt/LokSystem/loksystem --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/LokSystem/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/LokSystem/issues)
3. Create a [new issue](https://github.com/iOfficeAI/LokSystem/issues/new) with:
   - Your OS and version
   - LokSystem version
   - Steps to reproduce
...
**Happy using LokSystem in WebUI mode!** 🚀
