# LokSystem Headless Server Deployment Guide

Deploy LokSystem WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- LokSystem `.deb` package from [Releases](https://github.com/iOfficeAI/LokSystem/releases)
...
wget https://github.com/iOfficeAI/LokSystem/releases/latest/download/LokSystem-linux-amd64.deb

# Install
sudo dpkg -i LokSystem-linux-amd64.deb
...
LokSystem is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/LokSystem/start-loksystem.sh`:

```bash
#!/bin/bash
# LokSystem WebUI headless startup script
# Usage: ./start-loksystem.sh [start|stop|restart|status]
...
start() {
    ...
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        ...
        echo "LokSystem failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "LokSystem is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    ...
    pkill -f "LokSystem --webui" 2>/dev/null
    rm -f "$PIDFILE"
...
```bash
chmod +x /opt/LokSystem/start-loksystem.sh
```

> **Tip**: `WORKDIR` determines the directory LokSystem can access for file operations. Set it to your project workspace.

---

## Remote Access

LokSystem WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for LokSystem (Electron / Chromium Layer)
...
Create `/opt/LokSystem/proxy.pac`:
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/LokSystem --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/LokSystem/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: LokSystem Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside LokSystem WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by LokSystem's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i LokSystem-linux-amd64.deb`                 |
| LokSystem can only access `/tmp`             | Set `WORKDIR` in the startup script to your workspace path   |
...
在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 LokSystem WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- LokSystem `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/LokSystem/releases)）
...
LokSystem 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/LokSystem/start-loksystem.sh`：

```bash
#!/bin/bash
# LokSystem WebUI 无头启动脚本
# 用法: ./start-loksystem.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "LokSystem 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 LokSystem WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "LokSystem 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "LokSystem 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "LokSystem 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "LokSystem 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
LokSystem WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（LokSystem Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/LokSystem/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/LokSystem/proxy.pac"`
...
| LokSystem 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
