...
**LobeHub 能做到纯 JSON 是因为它的 agent 就只是一段 system prompt，无代码、无文件依赖。** LokSystem 的 contribution 类型更丰富（channelPlugin 需要 JS entryPoint, webui 需要 API routes, theme 需要 CSS 文件），有些天然需要打包在一起，强行拆开反而破坏内聚性。

**关键参考点:**

- LobeHub 的 Agent 市场证明了"按 contribution 粒度展示"的 UX 是成立的
- 但 LokSystem 不需要照搬"每种类型独立 repo"的模式，因为 contribution 间存在耦合（如 ext-feishu 的 channelPlugin + webui）
...
| Engine + Dependency 校验 | 安装前检查 LokSystem 版本兼容性 + 依赖扩展是否已安装                                    |
