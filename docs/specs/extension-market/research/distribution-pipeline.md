...
APP 启动 / 定期 (每 24h)
  ...
  │       AND semver.satisfies(appVersion, remote.engines.loksystem)
  │       → 标记为 "可更新"
  │
  └── UI 展示 Update 按钮
```
...
- 后续可加: 在安装新版前备份旧版到 `~/.loksystem/extensions/.backup/`, 支持一键回退

**版本号规范**:

- 严格 semver (`major.minor.patch`)
- CI 校验: manifest 中的 `version` 必须大于上一次发布版本 (防止降版本覆盖)
- `engines.loksystem` 使用 semver range (如 `>=2.0.0`, `^2.1.0`)
...
async function verifyExtension(tgzBuffer: Buffer, bundleJson: string): Promise<boolean> {
  ...
  await verify(bundle, tgzBuffer, {
    ...
    certificateIdentityRegExp: /https:\/\/github\.com\/loksystem\/hub\/.*/,
  });
...
const trustedIdentities = [
  /https:\/\/github\.com\/loksystem\/hub\/.*/, // 官方仓库
...
客户端下载 extension
  ...
  │   └── 版本匹配 → 直接使用 (零网络)
  │
  ├── 2. 检查本地缓存 (~/.loksystem/cache/)
  │   └── 版本匹配 + integrity 验证 → 直接使用
  │
  ├── 3. 从 GitHub 下载 (主源)
  │   ├── 优先: jsDelivr CDN (国内有节点)
  │   │   https://cdn.jsdelivr.net/gh/loksystem/hub@main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   ├── 备选: GitHub Raw
  │   │   https://raw.githubusercontent.com/loksystem/hub/main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   └── 备选: GitHub Releases (如果迁移到 Releases)
  │       https://github.com/loksystem/hub/releases/download/v2026.03/ext-claude-code-1.2.0.tgz
  │
  ├── 4. 下载后
  │   ├── 验证 SHA-512 integrity
  │   ├── 验证 Sigstore 签名 (Phase 2)
  │   ├── 缓存到 ~/.loksystem/cache/
  │   └── 解压到 ~/.loksystem/extensions/ext-claude-code/
  │
  └── 5. 所有源失败
      └── 显示错误提示, 建议检查网络
```
...
| 缓存        | `~/.loksystem/cache/<name>-<version>.tgz`, 安装成功后保留 (方便回退) |
...
下载 .tgz 完成
  ...
  │
  ├── 3. 解压到临时目录
  │   └── ~/.loksystem/extensions/.tmp/ext-claude-code/
  │
  ├── 4. 二次校验
...
```mermaid
flowchart TD
  subgraph GitHub["GitHub (loksystem/hub 仓库)"]
    direction TB
    ExtDirs["extensions/\next-claude-code/\next-opencode/\n..."]
...
| 现有模块               | 对接方式                                                    |
| ---------------------- | ----------------------------------------------------------- |
| `ExtensionLoader`      | 安装到 `~/.loksystem/extensions/` 后, 现有扫描逻辑自动发现     |
...
| 步骤 | 内容                                                                                          | 依赖     |
| ---- | --------------------------------------------------------------------------------------------- | -------- |
| 1.1  | 创建 `loksystem/hub` GitHub 仓库, 迁入官方 extension                                             | —        |
...
| 1.6  | 客户端: ExtensionInstaller — 下载 .tgz → 验证 integrity → 解压 → 安装到 ~/.loksystem/extensions/ | —        |
