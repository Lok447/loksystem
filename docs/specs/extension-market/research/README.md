...
- Worker Thread 沙箱的消息路由已修复（PR [#1991](https://github.com/iOfficeAI/LokSystem/pull/1991)：Worker→Host RPC、事件转发、storage API），但 `createSandbox()` 尚未被实际调用（ChannelPlugin 和 Lifecycle hooks 仍在主进程裸跑，待迁移）。
