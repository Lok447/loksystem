...
| **远程** | 从 GitHub `loksystem/hub` 仓库拉取的 index.json 中，存在于远程但不在内置列表中的 extension | 需要网络 |
...
```json
{
  ...
  "extensions": {
    "aionext-auggie": {
      ...
      "version": "1.0.0",
      "description": "Integrates Augment Code as an ACP adapter in LokSystem.",
    ...
    "aionext-claude": {
      ...
      "version": "1.0.0",
      "description": "Integrates Anthropic's Claude Code CLI as an ACP adapter in LokSystem.",
      "author": "Aionui Official",
      "engines": {
        "loksystem": "^1.0.0"
      },
...
判断 Hub extension 的展示状态:
  ...
  extension 目录不存在 (~/.loksystem/extensions/aionext-xxx/)
...
| 状态信号           | 数据来源                                         | 说明           |
| ------------------ | ------------------------------------------------ | -------------- |
| extension 是否存在 | `ExtensionLoader` 扫描 `~/.loksystem/extensions/`   | 已有能力       |
...
| **主页 Assistants**      | 预设助手配置 `assistants` | 保持现有不变 |
...
APP 启动
  │
  ├── 1. ExtensionLoader 扫描 ~/.loksystem/extensions/ (现有逻辑)
  │      → 得到已安装 extension 列表
  │
...
用户点击 Install
  ...
  │
  ├── 4. 解压到临时目录
  │      └── ~/.loksystem/extensions/.tmp/aionext-xxx/
  │
  ├── 5. 校验 aion-extension.json 合法性
  │      └── 失败 → 清理临时目录, 状态 → install_failed
  │
  ├── 6. 原子移动到正式目录
  │      └── .tmp/aionext-xxx/ → ~/.loksystem/extensions/aionext-xxx/
  │
...
用户点击 Uninstall (更多菜单中)
  ...
  │      └── 失败 → 仅 warn 日志, 不阻塞卸载
  │
  ├── 5. 删除 ~/.loksystem/extensions/aionext-xxx/ 目录
  │
...
用户点击 Retry (在 install_failed 状态)
  │
  ├── 检查 ~/.loksystem/extensions/aionext-xxx/ 是否已存在
...
拉取 remote-index.json
  ├── 优先: GitHub Raw
  │   https://raw.githubusercontent.com/loksystem/hub/main/dist/index.json
  │
  ├── 备选: jsDelivr CDN
  │   https://cdn.jsdelivr.net/gh/loksystem/hub@main/dist/index.json
  │
  ├── 超时: 5s (不阻塞 APP 启动)
  ├── 重试: 1 次
  │
...
for (const hubAgent of hubAgents) {
  ...
  const extDir = `~/.loksystem/extensions/${hubAgent.name}/`;
