...
> 调研日期：2026-03-24
> 目的：为 LokSystem 支持远程 Agent 连接提供协议选型和认证方案参考
...
| **JWT (自签发/自验证)**          | Tabby, OpenHands, LokSystem WebUI   | 需要过期/刷新的场景    |   中   |
...
1. **现有基础**：LokSystem 已有完整的 OpenClaw Gateway WS 连接实现（`OpenClawGatewayConnection.ts`），包含重连、心跳、Session Key
...
3. LokSystem 已有 JWT 认证基础（WebUI），可在第二期复用

### 5.3 远程 OpenClaw 优先

OpenClaw 的 Gateway WS 协议已被 LokSystem 实现过一次（本地模式），远程模式的核心差异仅在于：
...
- 消息适配层将不同 Agent 协议转换为 LokSystem 内部 ACP 格式
