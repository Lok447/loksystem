...
function getRuntimeVersion() {
  const configured = process.env.LOKSYSTEM_BUN_VERSION;
  return configured && configured.trim() ? configured.trim() : 'latest';
}

function getCacheRootDir() {
  const custom = process.env.LOKSYSTEM_BUN_CACHE_DIR;
  ...
  if (process.platform === 'win32') {
    ...
    return path.join(localAppData, 'LokSystem', 'cache', 'bundled-bun');
  }

  if (process.platform === 'darwin') {
    return path.join(os.homedir(), 'Library', 'Caches', 'LokSystem', 'bundled-bun');
  }

  const xdgCacheHome = process.env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache');
  return path.join(xdgCacheHome, 'LokSystem', 'bundled-bun');
}
...
function downloadRuntimeIntoCache(cacheRuntimeDir, platform, arch, version, variant = 'default') {
  ...
  const tempRoot = path.join(os.tmpdir(), 'loksystem-bundled-bun', version, `${platform}-${arch}-${variant}`);
