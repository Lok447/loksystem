...
/**
 * Simplified build script for LokSystem
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      ...
      if (lokSystemRunning || electronRunning) {
        ...
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running LokSystem/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'LokSystem.exe');
    ...
    console.log('⚠️  Windows local build failed after LokSystem.exe was produced.');
    if (firstError) {
