...
#   LOG_DIR            Directory for log files (default: ~/Library/Logs/LokSystem)
...
LOG_DIR=${LOG_DIR:-$HOME/Library/Logs/LokSystem}
...
cleanup_labels() {
  ...
  local stale_wts
  stale_wts=$(find /tmp -maxdepth 1 \( -name 'loksystem-pr-*' -o -name 'aionui-pr-*' \) -type d 2>/dev/null || true)
  if [ -n "$stale_wts" ]; then
