 ...
 * parsing the electron-log file for [LokSystem:ready] / [LokSystem:init] /
 * [LokSystem:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [LokSystem:ready] marks
  ...
  // Parsed from [LokSystem:init] marks
  initTotalMs: number;
  // Parsed from [LokSystem:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'LokSystem-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'LokSystem', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'LokSystem', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'LokSystem', 'logs', `${today}.log`));
  }
...
// Matches: [LokSystem:ready] <label> +<ms>ms
// Matches: [LokSystem:init]  <label> +<ms>ms
// Matches: [LokSystem:process] <label> +<ms>ms
const MARK_REGEX = /\[LokSystem:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[LokSystem] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[LokSystem] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[LokSystem] Showing main window')) marks.logs.showingMainWindow = true;
  }
