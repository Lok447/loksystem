...
```
~/.loksystem-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/LokSystem/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/LokSystem --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/LokSystem/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/LokSystem --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/LokSystem --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.loksystem-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | iOfficeAI/LokSystem | GitHub repository (owner/repo)                                      |
