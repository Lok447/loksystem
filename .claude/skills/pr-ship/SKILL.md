...
WORKTREE_DIR="/tmp/loksystem-ship-${PR_NUMBER}"
...
- **Worktree path** — always `/tmp/loksystem-ship-<PR_NUMBER>` (distinct from pr-automation's `/tmp/loksystem-pr-*` and pr-verify's `/tmp/loksystem-verify-*`)
