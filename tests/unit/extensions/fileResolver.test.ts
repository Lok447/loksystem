...
describe('extensions/fileResolver', () => {
  ...
  beforeEach(async () => {
    extensionDir = await fs.mkdtemp(path.join(os.tmpdir(), 'loksystem-ext-file-resolver-'));
  ...
  it('应阻止通过符号链接读取扩展目录外的文件', async () => {
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), 'loksystem-ext-outside-'));
