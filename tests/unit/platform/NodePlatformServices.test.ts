...
describe('NodePlatformServices.paths', () => {
  ...
  it('getDataDir falls back to homedir/.loksystem-server', async () => {
    const { NodePlatformServices } = await import('../../../src/common/platform/NodePlatformServices');
    expect(new NodePlatformServices().paths.getDataDir()).toBe(path.join(os.homedir(), '.loksystem-server'));
  });
  ...
  it('getLogsDir falls back to homedir/.loksystem-server/logs', async () => {
    const { NodePlatformServices } = await import('../../../src/common/platform/NodePlatformServices');
    expect(new NodePlatformServices().paths.getLogsDir()).toBe(path.join(os.homedir(), '.loksystem-server', 'logs'));
  });
