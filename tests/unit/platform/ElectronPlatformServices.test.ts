...
describe('ElectronPlatformServices.paths.getLogsDir', () => {
  ...
  it('returns app.getPath("logs") when it succeeds', async () => {
    mockGetPath.mockImplementation((name: string) => {
      if (name === 'logs') return '/Users/test/Library/Logs/LokSystem';
      if (name === 'userData') return '/Users/test/Library/Application Support/LokSystem';
      return `/mock/${name}`;
    });
    ...
    expect(svc.paths.getLogsDir()).toBe('/Users/test/Library/Logs/LokSystem');
  });

  it('falls back to userData/logs when app.getPath("logs") throws', async () => {
    const userData = '/Users/test/Library/Application Support/LokSystem';
    mockGetPath.mockImplementation((name: string) => {
