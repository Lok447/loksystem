...
// from running LokSystem instances, causing port conflicts. Back it up and restore.
const REAL_REGISTRY = path.join(os.homedir(), '.loksystem-cdp-registry.json');
let savedRegistry: string | null = null;

function createSandbox(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'loksystem-cdp-test-'));
}
...
async function loadConfigureChromium(options: SetupOptions = {}) {
  ...
  const registryPath = path.join(sandbox, '.loksystem-cdp-registry.json');
  ...
  delete process.env.LOKSYSTEM_CDP_PORT;
  if (options.envPort !== undefined) {
    process.env.LOKSYSTEM_CDP_PORT = options.envPort;
  }
...
describe('configureChromium CDP (lightweight mock + file sandbox)', () => {
  ...
  describe('dev environment isolation', () => {
    it('sets app name and userData path in dev mode', async () => {
      ...
      expect(ctx.setNameSpy).toHaveBeenCalledWith('LokSystem-Dev');
      expect(ctx.setPathSpy).toHaveBeenCalledWith('userData', path.join(ctx.sandbox, 'LokSystem-Dev'));
    });
