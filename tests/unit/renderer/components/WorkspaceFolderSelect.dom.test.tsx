...
describe('WorkspaceFolderSelect', () => {
  ...
  it('opens downward when there is enough space below the trigger', () => {
    localStorage.setItem('loksystem:recent-workspaces', JSON.stringify(['/tmp/a']));
...
describe('WorkspaceFolderSelect - browse interactions', () => {
  ...
  it('saves the chosen path to recent workspaces after browse', async () => {
    ...
    fireEvent.click(screen.getByTestId('ws-trigger'));
    await waitFor(() => expect(mockShowOpen).toHaveBeenCalled());
    const stored = JSON.parse(localStorage.getItem('loksystem:recent-workspaces') ?? '[]');
    expect(stored).toContain('/chosen/path');
  });

  it('opens browse picker via the "choose different" button inside the menu', async () => {
    localStorage.setItem('loksystem:recent-workspaces', JSON.stringify(['/old']));
  ...
  it('selects a recent workspace and closes the menu', () => {
    localStorage.setItem('loksystem:recent-workspaces', JSON.stringify(['/tmp/project-a', '/tmp/project-b']));
    const onChange = vi.fn();
    render(
      <WorkspaceFolderSelect
        value=''
  ...
  it('highlights the selected recent workspace and shows a check icon', () => {
    localStorage.setItem('loksystem:recent-workspaces', JSON.stringify(['/tmp/selected', '/tmp/other']));
    render(
      <WorkspaceFolderSelect
        value='/tmp/selected'
        onChange={vi.fn()}
  ...
  it('handles corrupted localStorage data gracefully', () => {
    localStorage.setItem('loksystem:recent-workspaces', 'not-valid-json{');
    render(
      <WorkspaceFolderSelect
        value=''
        onChange={vi.fn()}
...
describe('WorkspaceFolderSelect - outside click', () => {
  ...
  it('closes the dropdown when clicking outside', () => {
    localStorage.setItem('loksystem:recent-workspaces', JSON.stringify(['/tmp/ws']));
    render(
      <WorkspaceFolderSelect
        value=''
        onChange={vi.fn()}
