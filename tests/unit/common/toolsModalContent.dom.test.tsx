...
const testState = vi.hoisted(() => ({
  ...
  initialBuiltinServer: (): MockServer => ({
    ...
    name: 'loksystem-image-generation',
    builtin: true,
    enabled: false,
    updatedAt: 1,
    transport: {
      ...
      env: {
        LOKSYSTEM_IMG_PLATFORM: 'new-api',
        LOKSYSTEM_IMG_BASE_URL: 'https://example.com',
        LOKSYSTEM_IMG_API_KEY: 'key',
        LOKSYSTEM_IMG_MODEL: 'grok-imagine-1.0',
      },
    },
  }),
  mockConfigGet: vi.fn(),
...
vi.mock('@/renderer/components/base/LokSelect', () => {
  const Select = ({
    children,
    value,
    onChange,
...
describe('ToolsModalContent image generation status refresh', () => {
  ...
  it('refreshes image generation agent status only after sync completes when enabling the builtin MCP server', async () => {
    ...
    expect(testState.mockCheckSingleServerInstallStatus).toHaveBeenCalledWith('loksystem-image-generation');
  });
