...
describe('prepareBundledBun', () => {
  ...
  const originalCacheDir = process.env.LOKSYSTEM_BUN_CACHE_DIR;
  const originalVersion = process.env.LOKSYSTEM_BUN_VERSION;
  ...
  afterEach(() => {
    process.env.LOKSYSTEM_BUN_CACHE_DIR = originalCacheDir;
    process.env.LOKSYSTEM_BUN_VERSION = originalVersion;
  ...
  function setupCacheAndBackup(version: string) {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'loksystem-bun-test-'));
    ...
    process.env.LOKSYSTEM_BUN_CACHE_DIR = cacheRoot;
    process.env.LOKSYSTEM_BUN_VERSION = version;
