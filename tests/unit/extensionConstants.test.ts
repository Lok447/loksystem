...
vi.mock('@process/utils', () => ({
  getDataPath: () => '/home/user/.loksystem-dev',
}));
...
describe('extension constants', () => {
  ...
  describe('HUB_REMOTE_URLS with LOKSYSTEM_HUB_URL env var', () => {
    const originalEnv = process.env.LOKSYSTEM_HUB_URL;

    afterEach(() => {
      if (originalEnv === undefined) delete process.env.LOKSYSTEM_HUB_URL;
      else process.env.LOKSYSTEM_HUB_URL = originalEnv;
      vi.resetModules();
    });

    it('should return only default URLs when env var is not set', async () => {
      delete process.env.LOKSYSTEM_HUB_URL;
      vi.resetModules();
    ...
    it('should prepend custom URLs from env var', async () => {
      process.env.LOKSYSTEM_HUB_URL = 'http://localhost:3000/';
      vi.resetModules();
      const { HUB_REMOTE_URLS: urls } = await import('../../src/process/extensions/constants');
    ...
    it('should support comma-separated URLs', async () => {
      process.env.LOKSYSTEM_HUB_URL = 'http://a.com/,http://b.com/';
      vi.resetModules();
      const { HUB_REMOTE_URLS: urls } = await import('../../src/process/extensions/constants');
  ...
  describe('getUserExtensionsDir', () => {
    it('should return data path + extensions', () => {
      expect(getUserExtensionsDir()).toBe(path.join('/home/user/.loksystem-dev', 'extensions'));
    });
  });
  ...
  describe('getEnvExtensionsDirs', () => {
    const originalEnv = process.env.LOKSYSTEM_EXTENSIONS_PATH;

    afterEach(() => {
      if (originalEnv === undefined) {
        delete process.env.LOKSYSTEM_EXTENSIONS_PATH;
      } else {
        process.env.LOKSYSTEM_EXTENSIONS_PATH = originalEnv;
      }
    });
    ...
    it('should split env var by path separator', () => {
      ...
      process.env.LOKSYSTEM_EXTENSIONS_PATH = `/path/a${sep}/path/b`;
      expect(getEnvExtensionsDirs()).toEqual(['/path/a', '/path/b']);
    });

    it('should filter out empty segments', () => {
      const sep = process.platform === 'win32' ? ';' : ':';
      process.env.LOKSYSTEM_EXTENSIONS_PATH = `/path/a${sep}${sep}/path/b`;
  ...
  describe('getHubResourcesDir', () => {
    ...
    it('should return process resources path in packaged mode', () => {
      ...
      Object.defineProperty(process, 'resourcesPath', {
        value: '/opt/LokSystem/resources',
        configurable: true,
        writable: true,
      });

      expect(getHubResourcesDir()).toBe(path.join('/opt/LokSystem/resources', 'hub'));
    });
  });

  describe('getExtensionScanSources', () => {
    const originalEnv = process.env.LOKSYSTEM_EXTENSIONS_PATH;
    const originalE2E = process.env.LOKSYSTEM_E2E_TEST;

    afterEach(() => {
      if (originalEnv === undefined) delete process.env.LOKSYSTEM_EXTENSIONS_PATH;
      else process.env.LOKSYSTEM_EXTENSIONS_PATH = originalEnv;
      if (originalE2E === undefined) delete process.env.LOKSYSTEM_E2E_TEST;
      else process.env.LOKSYSTEM_E2E_TEST = originalE2E;
    });

    it('should return user and appdata dirs by default', () => {
      delete process.env.LOKSYSTEM_EXTENSIONS_PATH;
      delete process.env.LOKSYSTEM_E2E_TEST;
    ...
    it('should include env dirs with highest priority when set', () => {
      process.env.LOKSYSTEM_EXTENSIONS_PATH = '/custom/ext';
      delete process.env.LOKSYSTEM_E2E_TEST;
      const sources = getExtensionScanSources();
    ...
    it('should only include env dirs in E2E mode', () => {
      process.env.LOKSYSTEM_EXTENSIONS_PATH = '/e2e/ext';
      process.env.LOKSYSTEM_E2E_TEST = '1';
      const sources = getExtensionScanSources();
  ...
  describe('getInstallTargetDir', () => {
    ...
    it('should return env dir when LOKSYSTEM_EXTENSIONS_PATH is set', () => {
      process.env.LOKSYSTEM_EXTENSIONS_PATH = '/my/ext';
      delete process.env.LOKSYSTEM_E2E_TEST;
      const dir = getInstallTargetDir();
