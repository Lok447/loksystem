...
describe('Step 6: loadBuiltinSessionMcpServers — builds servers list', () => {
  ...
  it('returns [teamServer] when teamMcpStdioConfig is set', async () => {
    ...
    expect(servers[0]).toMatchObject({ name: 'loksystem-team-abc', command: 'node' });
...
describe('Step 7a: createOrResumeSession — Codex vs non-Codex routing', () => {
  ...
  it('non-Codex resume: newSession options contain mcpServers array', async () => {
    ...
    expect(opts.mcpServers[0]).toMatchObject({ name: 'loksystem-team-abc' });
  });
...
describe('Step 7b PROOF-OF-FIX: Codex loadSession receives mcpServers (Task #1)', () => {
  ...
  it('PROOF-OF-FIX: loadSession receives team MCP server when teamMcpStdioConfig is set', async () => {
    ...
    // On fixed:   args[2] = [{name:'loksystem-team-abc',...}] (direct array) → PASSES
    expect(args[2]).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'loksystem-team-abc', command: 'node' })])
    );
...
describe('Step 8: Task #3 IPC mcpStatus events', () => {
  ...
  it('emits degraded when team config is missing but mcpServers ends up empty', async () => {
    ...
    const agent = createCodexAgent({
      teamMcpStdioConfig: { name: 'loksystem-team-abc', command: '', args: [], env: [] },
    });
    await callCreateOrResume(agent);
  ...
  it('event payload includes teamId derived from server name', async () => {
    ...
    // teamId extracted from 'loksystem-team-abc' → 'abc'
...
//
// How to verify "frontend can see MCP injection status" in a real LokSystem env:
//
// HAPPY PATH:
//   1. Launch LokSystem, open a Team with at least one ACP (claude/codex) agent
...
//
// DIAGNOSIS COMMANDS:
//   grep "Injecting team MCP server" ~/Library/Logs/LokSystem/*.log
//   sqlite3 ~/Library/.../loksystem.db "SELECT extra FROM conversations WHERE id='<agentConvId>'"
