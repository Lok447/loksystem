...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'loksystem-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'loksystem-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      LOKSYSTEM_EXTENSIONS_PATH: emptyExtensionsDir,
      LOKSYSTEM_EXTENSION_STATES_FILE: extensionStatesFile,
