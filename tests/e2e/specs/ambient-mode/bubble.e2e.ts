 ...
 *   AC-M1-11 LOKSYSTEM_AMBIENT env var 优先级 > settings 开关
 ...
 */
// AC-M1-14 fixture: use ambientTest (independent LOKSYSTEM_AMBIENT=1 Electron process,
...
test.describe('Ambient Mode — M1 Bubble', () => {
  ...
  test.beforeAll(async ({ electronApp }) => {
    ...
    test.skip(
      ...
      'Ambient bubble window not found. Blockers: (1) Dev has not implemented ambient mode yet; ' +
        '(2) Q7 REQ-CLARIFY-REPLY pending — `LOKSYSTEM_AMBIENT=1` launch path under singleton fixture undefined. ' +
  ...
  // ── AC-M1-11: LOKSYSTEM_AMBIENT env var 优先级高于 settings 开关 ─────────
  test('AC-M1-11: LOKSYSTEM_AMBIENT env var overrides settings switch', async ({ electronApp }) => {
    test.skip(
      true,
      'PENDING AC-M1-14 fixture `launchAppWithEnv` + `ambientTest`: this assertion requires launching a second ' +
        'Electron process with `LOKSYSTEM_AMBIENT=0` while ConfigStorage `ambient.enabled=true` (and the reverse), ' +
  ...
  test('AC-M1-13: invalid persisted displayId falls back to primary display default', async () => {
    test.skip(
      ...
      'PENDING AC-M1-14 fixture `launchAppWithEnv`: this assertion requires seeding ConfigStorage with ' +
        ...
        'Assertion shape: write stale displayId → launchAppWithEnv({LOKSYSTEM_AMBIENT:"1"}) → getAmbientBubbleInfo → ' +
