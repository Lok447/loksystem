 ...
 *      (e.g. out/linux-unpacked/loksystem, out/mac-arm64/LokSystem.app, out/win-unpacked/LokSystem.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'loksystem-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/LokSystem.exe  or  out/win-x64-unpacked/LokSystem.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'LokSystem.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/LokSystem.app/Contents/MacOS/LokSystem  or  out/mac/LokSystem.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'LokSystem');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/loksystem  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['loksystem', 'LokSystem']) {
 ...
 * process with `LOKSYSTEM_AMBIENT=1` while the singleton remains ambient-unaware.
 * Regular (non-ambient) tests call this without `extraEnv` via the singleton path.
 *
 * @param extraEnv — merged on top of `commonEnv` (e.g. `{ LOKSYSTEM_AMBIENT: '1' }`).
...
export async function launchAppWithEnv(extraEnv: Record<string, string> = {}): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    LOKSYSTEM_EXTENSIONS_PATH: process.env.LOKSYSTEM_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    LOKSYSTEM_EXTENSION_STATES_FILE: process.env.LOKSYSTEM_EXTENSION_STATES_FILE || e2eStateFile,
    LOKSYSTEM_DISABLE_AUTO_UPDATE: '1',
    LOKSYSTEM_DISABLE_DEVTOOLS: '1',
    LOKSYSTEM_E2E_TEST: '1',
    LOKSYSTEM_CDP_PORT: '0',
    ...extraEnv,
  };
  ...
  if (usePackaged) {
    ...
    console.log(
      `[E2E] Launching PACKAGED app: ${packaged.executablePath}${extraEnv.LOKSYSTEM_AMBIENT === '1' ? ' (ambient)' : ''}`
    );
    ...
    const expectAmbient = commonEnv.LOKSYSTEM_AMBIENT === '1';
  ...
  console.log(`[E2E] Launching DEV app from: ${projectRoot}${extraEnv.LOKSYSTEM_AMBIENT === '1' ? ' (ambient)' : ''}`);
  ...
  //
  // When LOKSYSTEM_AMBIENT=1 is set, we specifically wait for the ambient
...
// `LOKSYSTEM_AMBIENT=1` env var (AC-M1-11 feature flag — env var overrides
...
export const ambientTest = base.extend<AmbientFixtures>({
  ...
  ambientApp: async ({}, use) => {
    if (!sharedAmbientApp) {
      sharedAmbientApp = await launchAppWithEnv({ LOKSYSTEM_AMBIENT: '1' });
    }
    // Verify the app process is still alive; relaunch if it crashed
    try {
