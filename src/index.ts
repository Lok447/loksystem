  ...
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[LokSystem] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  ...
  console.log(`[LokSystem] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[LokSystem] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[LokSystem] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[LokSystem] Renderer did-finish-load');
      showWindow();
    });
  ...
  const disableAutoUpdater =
    process.env.LOKSYSTEM_DISABLE_AUTO_UPDATE === '1' || process.env.LOKSYSTEM_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[LokSystem] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[LokSystem] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[LokSystem] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[LokSystem] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[LokSystem] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[LokSystem] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[LokSystem] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[LokSystem] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[LokSystem] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[LokSystem] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[LokSystem] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[LokSystem] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[LokSystem] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  try {
    ...
    console.error('[LokSystem] Failed to restore zoom factor:', error);
  ...
  if (isResetPasswordMode) {
    ...
    //
    // Priority:
    //   1. LOKSYSTEM_AMBIENT=1 → ambient  (env wins)
    //   2. LOKSYSTEM_AMBIENT=0 → pet      (env wins, explicit off)
    ...
    const ambientEnvVar = process.env['LOKSYSTEM_AMBIENT'];
    void (async () => {
      try {
...
// Register loksystem:// as the default protocol client
...
// macOS: handle loksystem:// URLs via the open-url event
...
app.on('will-quit', () => {
  console.log('[LokSystem] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[LokSystem] quit (exitCode=${exitCode})`);
});
