      ...
      {menus.map((item, index) => {
        ...
        const groupHeader =
          groupHeaderKey && !collapsed ? (
            <div className='settings-sider__group-header px-12px pt-8px pb-0 h-28px flex items-center text-12px font-normal text-t-tertiary select-none'>
              {t(groupHeaderKey)}
            </div>
          ) : null;
        return (
            ...
            <Tooltip {...siderTooltipProps} content={item.label} position='right'>
              ...
              >
                ...
                <FlexFullContainer className='h-24px collapsed-hidden'>
                  <div
                    className='settings-sider__item-label text-nowrap overflow-hidden inline-block w-full text-14px lh-24px whitespace-nowrap text-t-primary'
                  >
                    {item.label}
                  </div>
                </FlexFullContainer>
