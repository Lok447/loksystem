...
const LokLogoMark: React.FC = () => (
  <img className='app-titlebar__brand-logo' src={lokLogo} alt='' aria-hidden='true' draggable={false} />
);
...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const appTitle = useMemo(() => 'LokSystem', []);
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('lok:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('lok:last-non-settings-path');
      if (stored) {
