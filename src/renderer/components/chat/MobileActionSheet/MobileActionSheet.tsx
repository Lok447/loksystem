...
const MobileActionSheet: React.FC<MobileActionSheetProps> = ({ open, onClose, title, entries }) => {
  ...
  const activeEntry = activeSubKey ? entries.find((entry) => entry.key === activeSubKey) : null;
  const activeSub: MobileActionSheetSubMenu | undefined = activeEntry?.submenu;
  const renderedSubEntry = renderedSubKey ? entries.find((entry) => entry.key === renderedSubKey) : null;
                  ...
                  >
                    ...
                    {(entry.meta || entry.submenu) && (
                      <div className={styles.meta}>
                        ...
                        {entry.submenu && <Right theme='outline' size='14' className={styles.chevron} aria-hidden='true' />}
                      </div>
                    )}
                  </div>
                </Fragment>
              ...
              <div className={styles.list}>
                {renderedSub.options.length === 0 ? (
                  ...
                  renderedSub.options.map((option) => {
                    ...
                    return (
                      <div key={option.key} className={styles.item} onClick={() => handleSubSelect(option.key)}>
                        <div className={styles.body}>
