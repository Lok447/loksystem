...
declare global {
  interface Window {
    __LokSafeResizeObserver__?: boolean;
    __LokResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__LokSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__LokSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__LokResizeObserverPatched__) {
    ...
    window.__LokResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__LokResizeObserverPatched__) {
    ...
    console.__LokResizeObserverPatched__ = true;
  }
};
