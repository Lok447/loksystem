import { LOKSYSTEM_FILES_MARKER, LOKSYSTEM_TIMESTAMP_REGEX } from '@/common/config/constants';
...
export const buildDisplayMessage = (input: string, files: string[], workspacePath: string): string => {
  ...
  const displayPaths = files.map((filePath) => {
    const sanitizedPath = filePath.replace(LOKSYSTEM_TIMESTAMP_REGEX, '$1');
    ...
    if (isAbsolute) {
      ...
      if (normalizedFile.startsWith(normalizedWorkspaceWithForwardSlash + '/')) {
        ...
        return `${normalizedWorkspace}/${relativePath.replace(LOKSYSTEM_TIMESTAMP_REGEX, '$1')}`;
      }
