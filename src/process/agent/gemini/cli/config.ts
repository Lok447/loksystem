...
export async function loadCliConfig({
  ...
  const builtinSkillsExtension: GeminiCLIExtension = {
    name: 'loksystem-builtin-skills',
    version: '1.0.0',
    isActive: true,
    path: skillsDir || '',
    contextFiles: [],
    id: 'loksystem-builtin-skills',
  ...
  if (!argv.allowedMcpServerNames) {
    if (settings.allowMCPServers) {
      ...
      if (allowedNames.size > 0) {
        mcpServersConfig = Object.fromEntries(
          // Team MCP servers (loksystem-team-*) are always kept regardless of allowlist
          Object.entries(mcpServersConfig).filter(([key]) => key.startsWith('loksystem-team-') || allowedNames.has(key))
        );
      }
    }
    ...
    if (settings.excludeMCPServers) {
      ...
      if (excludedNames.size > 0) {
        mcpServersConfig = Object.fromEntries(
          // Team MCP servers (loksystem-team-*) are never excluded
          Object.entries(mcpServersConfig).filter(([key]) => key.startsWith('loksystem-team-') || !excludedNames.has(key))
        );
      }
    }
  }
  ...
  const config = new Config({
    ...
    // LokSystem 是桌面应用，支持用户交互确认，需要设置 interactive: true
    // LokSystem is a desktop app with user interaction support, needs interactive: true
    interactive: true,
