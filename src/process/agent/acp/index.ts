...
export class AcpAgent {
  ...
  private async createOrResumeSession(): Promise<void> {
    ...
    // Derive teamId from injected team MCP server name (format: loksystem-team-<teamId>)
    // Only emit MCP status events when running inside a team session.
    const teamMcpName = this.extra.teamMcpStdioConfig?.name;
    const teamId = teamMcpName?.startsWith('loksystem-team-') ? teamMcpName.slice('loksystem-team-'.length) : undefined;
  ...
  private async loadBuiltinSessionMcpServers(): Promise<AcpSessionMcpServer[]> {
    try {
      ...
      const tId =
        typeof mcpName === 'string' && mcpName.startsWith('loksystem-team-')
