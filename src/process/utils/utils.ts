...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'loksystem');
};
 ...
 * Release builds use ~/.loksystem; dev builds use ~/.loksystem-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.loksystem，Dev 模式使用 ~/.loksystem-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'loksystem');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.loksystem'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.loksystem-config; dev builds use ~/.loksystem-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.loksystem-config，Dev 模式使用 ~/.loksystem-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.loksystem-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[LokSystem] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[LokSystem] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[LokSystem] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[LokSystem] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
