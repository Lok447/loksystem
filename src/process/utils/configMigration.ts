...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.loksystem-config', 'loksystem-config.txt'),
      path.join(home, '.loksystem-config-dev', 'loksystem-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, 'LokSystem', 'config', 'loksystem-config.txt'),
      path.join(appData, 'LokSystem-Dev', 'config', 'loksystem-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', 'LokSystem', 'config', 'loksystem-config.txt'),
    path.join(home, '.config', 'LokSystem-Dev', 'config', 'loksystem-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[LokSystem] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[LokSystem] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[LokSystem] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an loksystem-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[LokSystem] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[LokSystem] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[LokSystem] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[LokSystem] IMPORT_CONFIG_FROM failed:', error);
  }
}
