...
const STORAGE_PATH = {
  config: 'loksystem-config.txt',
  chatMessage: 'loksystem-chat-message.txt',
  chat: 'loksystem-chat.txt',
  env: '.loksystem-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    console.error('[LokSystem] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('loksystem.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'loksystem-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'loksystem-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'loksystem-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'loksystem-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[LokSystem] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
  ...
  if (existsSync(builtinSkillsDir)) {
    try {
      ...
      console.warn(`[LokSystem] Failed to sync builtin skills directory:`, error);
    }
  }
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[LokSystem] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[LokSystem] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[LokSystem] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
...
const ensureBuiltinMcpServers = async (): Promise<void> => {
  try {
    ...
    const buildEnvFromConfig = (cfg: typeof oldConfig): Record<string, string> => {
      ...
      if (cfg.platform) env.LOKSYSTEM_IMG_PLATFORM = cfg.platform;
      if (cfg.baseUrl) env.LOKSYSTEM_IMG_BASE_URL = cfg.baseUrl;
      if (cfg.apiKey) env.LOKSYSTEM_IMG_API_KEY = cfg.apiKey;
      if (cfg.useModel) env.LOKSYSTEM_IMG_MODEL = cfg.useModel;
      return env;
    };
    ...
    if (changed) {
      ...
      console.log('[LokSystem] Built-in MCP servers ensured');
    }
    ...
    console.error('[LokSystem] Failed to ensure built-in MCP servers:', error);
  }
};
...
const cleanupOrphanedHealthCheckConversations = async () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[LokSystem] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[LokSystem] Failed to cleanup orphaned health-check conversations:', error);
  }
};
...
const initStorage = async () => {
  ...
  try {
    ...
    console.error('[LokSystem] Failed to initialize default MCP servers:', error);
  }
...
export const loadSkillsContent = async (enabledSkills: string[]): Promise<string> => {
  ...
  for (const skillName of enabledSkills) {
    ...
    try {
      ...
      console.warn(`[LokSystem] Failed to load skill ${skillName}:`, error);
    }
  }
