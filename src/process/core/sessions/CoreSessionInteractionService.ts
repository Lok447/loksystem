...
export class CoreSessionInteractionService {
  constructor(private readonly conversationService: IConversationService) {}

  public async askSideQuestion(conversationId: string, question: string): Promise<ConversationSideQuestionResult> {
  ...
  private async askWithAcpFork(
      ...
      },
      {
        onSessionUpdate: (notification) => {
          ...
          if (update.sessionUpdate === 'tool_call' || update.sessionUpdate === 'tool_call_update') {
            console.warn('[CoreSessionInteractionService] ACP /btw cancelled due to tool activity', {
        ...
        onRequestPermission: async (request) => {
          console.warn('[CoreSessionInteractionService] ACP /btw rejected permission request', {
            backend: context.backend,
            conversationId,
          ...
          const rejectOption = request.options.find((option) => option.kind.startsWith('reject'));
          return {
            outcome: {
    ...
    client.onDisconnect((info) => {
      console.warn('[CoreSessionInteractionService] ACP /btw runner disconnected unexpectedly', {
        backend: context.backend,
        conversationId,
        reason: info.reason,
    ...
    try {
      ...
      await client.close().catch((error: unknown) => {
        console.warn('[CoreSessionInteractionService] Failed to close ACP /btw runner', {
          backend: context.backend,
          conversationId,
