...
const DEFAULT_REPO = 'iOfficeAI/LokSystem';
const DEFAULT_USER_AGENT = 'LokSystem';
const ALLOWED_ASSET_EXTS = new Set(['.exe', '.msi', '.dmg', '.zip', '.deb', '.rpm']);
const CDN_HOST = 'static.loksystem.com';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.LOKSYSTEM_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `LokSystem-update-${Date.now()}`;
};
