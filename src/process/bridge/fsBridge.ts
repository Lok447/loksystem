...
export function initFsBridge(): void {
  ...
  ipcBridge.fs.createTempFile.provider(async ({ fileName }) => {
    try {
      ...
      if (fileExists) {
        ...
        const tempFileName = `${name}${LOKSYSTEM_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  ipcBridge.fs.createUploadFile.provider(async ({ fileName, conversationId, workspace }) => {
    try {
      ...
      if (fileExists) {
        ...
        const ext = path.extname(safeFileName);
        const name = path.basename(safeFileName, ext);
        const newFileName = `${name}${LOKSYSTEM_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  // Skills Market: inject the loksystem-skills builtin skill
  ipcBridge.fs.enableSkillsMarket.provider(async () => {
    try {
      const { getAutoSkillsDir } = await import('@process/utils/initStorage');
      const skillDir = path.join(getAutoSkillsDir(), 'loksystem-skills');
  ...
  // Skills Market: remove the loksystem-skills builtin skill
...
/**
 * Read the bundled SKILL.md for loksystem-skills from app resources.
 *
...
async function readBundledSkillsMarketMd(): Promise<string> {
  try {
    const fallbackPath = path.join(getBuiltinSkillsCopyDir(), 'loksystem-skills', 'SKILL.md');
