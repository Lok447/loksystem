...
export const CSRF_COOKIE_NAME = 'loksystem-csrf-token';
...
export const AUTH_CONFIG = {
  ...
  COOKIE: {
    ...
    NAME: 'loksystem-session' as const,
    OPTIONS: {
 ...
 *
 * 信号来源（按优先级）：
 * 1. LOKSYSTEM_HTTPS=true 或 NODE_ENV=production + HTTPS=true（显式开关）
 ...
 *
 * Signals (by priority):
 * 1. LOKSYSTEM_HTTPS=true / NODE_ENV=production + HTTPS=true (explicit opt-in)
...
function detectHttps(req?: Request): boolean {
  if (process.env.LOKSYSTEM_HTTPS === 'true' || (process.env.NODE_ENV === 'production' && process.env.HTTPS === 'true')) {
    return true;
  }
