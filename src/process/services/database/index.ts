...
/**
 * Main database class for LokSystem
 * Uses a pluggable ISqliteDriver for SQLite operations
 */
export class LokSystemDatabase {
  ...
  /**
   * Create a new LokSystemDatabase instance with corruption recovery.
   * This is the only way to obtain an instance — the constructor is private.
   */
  static async create(dbPath: string): Promise<LokSystemDatabase> {
    ...
    try {
      ...
      failedDriver = driver;
      const instance = new LokSystemDatabase(driver);
      instance.initialize();
      return instance;
    } catch (error) {
   ...
   * Prefer using dedicated methods on LokSystemDatabase where possible.
   */
...
let dbInstancePromise: Promise<LokSystemDatabase> | null = null;
// Synchronous reference to the resolved instance — used for safe close on exit
let dbResolved: LokSystemDatabase | null = null;

function resolveDbPath(): string {
  return path.join(getDataPath(), 'loksystem.db');
}

export function getDatabase(): Promise<LokSystemDatabase> {
  if (!dbInstancePromise) {
    dbInstancePromise = LokSystemDatabase.create(resolveDbPath()).then((db) => {
      dbResolved = db;
      return db;
    });
  }
