...
export class WorkspaceSnapshotService {
  ...
  /**
   * Remove leftover `loksystem-snapshot-*` directories from previous sessions
  ...
  static async cleanupStaleSnapshots(): Promise<void> {
    ...
    const stale = entries.filter((name) => name.startsWith('loksystem-snapshot-'));
  ...
  private async createWorkingTreeSnapshot(workspacePath: string): Promise<string> {
    const gitdir = path.join(os.tmpdir(), `loksystem-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
