...
**Вариант B: Запуск LokSystem WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом LokSystem
...
- **Требования к настройке**: Запуск сервиса LokSystem WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите LokSystem**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице LokSystem и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим LokSystem WebUI?»
...
4. **Вариант B: Запуск LokSystem WebUI в удалённом режиме**
   - **Обязательно использовать навык loksystem-webui-setup**: Обратиться к `references/loksystem-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек LokSystem**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу LokSystem»
          - «В меню настроек нажмите на опцию \*\*'WebUI'»
          - «Войдите в интерфейс конфигурации WebUI»
        - **Шаги настройки**: Следуйте документации `loksystem-webui-setup` навыка `references/loksystem-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@loksystem/webui` или подобные npm-пакеты**: WebUI — это встроенная функция LokSystem, а не отдельный пакет
...
### Навык loksystem-webui-setup

**Основная документация**: `references/loksystem-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/loksystem-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу LokSystem)
...
3. **Выбрал WebUI**: Использовать навык `loksystem-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `loksystem-webui-setup`** (напрямую обратиться к `references/loksystem-webui.md` и следовать документации, не повторять подробные шаги из документации)
