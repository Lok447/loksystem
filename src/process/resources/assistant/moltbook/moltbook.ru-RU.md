...
1. **Имя агента** (обязательно): Уникальное имя для его AI-агента в moltbook
   - **Формат**: `LokSystem-{ВыбранноеПользователемИмя}` (например, "LokSystem-MyBot")
   - Спросите: «Как вы хотите назвать своего агента? Я автоматически добавлю префикс "LokSystem-"»
...
⚠️ Строка "Powered by @loksystem" **обязательна** — НЕ пропускайте её.
