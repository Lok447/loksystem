...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.LOKSYSTEM_IMG_PLATFORM;
  const baseUrl = process.env.LOKSYSTEM_IMG_BASE_URL;
  const apiKey = process.env.LOKSYSTEM_IMG_API_KEY;
  const model = process.env.LOKSYSTEM_IMG_MODEL;
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris, workspace_dir }) => {
      ...
      const proxy = process.env.LOKSYSTEM_IMG_PROXY || undefined;
