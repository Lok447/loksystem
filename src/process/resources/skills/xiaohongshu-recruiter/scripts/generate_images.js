...
// Priority: 1) LOKSYSTEM_FONTS_DIR env var, 2) skills/canvas-design relative path, 3) system fonts
function getFontDir() {
  const candidates = [
    process.env.LOKSYSTEM_FONTS_DIR,
    path.join(__dirname, '../../canvas-design/canvas-fonts'),
    path.join(process.env.HOME || '', 'Library/Application Support/LokSystem/config/skills/canvas-design/canvas-fonts'),
    path.join(process.env.APPDATA || '', 'LokSystem/config/skills/canvas-design/canvas-fonts'),
  ].filter(Boolean);
